...
public class Player : MonoBehaviour {
    ...
    private void TryToUnStuck() {
        ...
        if(UnstuckTries >= random) {
            ...
            UnstuckTries = 0;
            Rigidbody.AddForce(UnStuckDirection * MoveForce, ForceMode2D.Impulse);
        }
    }
        ...
        if(Input.touchCount > 0 && Input.GetTouch(0).phase == UnityEngine.TouchPhase.Ended) {
            ...
            if(Mathf.Abs(inputVector.x) > Mathf.Abs(inputVector.y)) {
                if(inputVector.x > 0) {
                    ...
                    MovePlayerOnce(Vector2.left);
                }
            }
            else {
                if(inputVector.y > 0) {
                    MovePlayerOnce(Vector2.up);
                }
                else {
    ...
    private void MovePlayerOnce(Vector2 direction) {
